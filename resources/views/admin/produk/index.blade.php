...
@if(session('success'))
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 pb-4 border-b border-amber-200/60 gap-4">
        <div>
            ...
            <p class="text-sm text-gray-500">Atur daftar ukuran varian, harga, dan pantau stok kopi Wasana Coffee.</p>
        </div>
        <a href="{{ route('admin.produk.create') }}" class="inline-flex items-center bg-amber-800 hover:bg-amber-900 text-white font-semibold py-2.5 px-5 rounded-xl transition shadow gap-2 text-sm">
            ➕ Tambah Varian Baru
        </a>
    </div>
            ...
            <tbody class="text-gray-600 text-sm divide-y divide-amber-50">
                @forelse($produks as $index => $item)
                    <tr class="hover:bg-amber-50/30 transition">
                        <td class="p-4 pl-6 font-medium text-gray-400">{{ $index + 1 }}</td>
                        <td class="p-4 font-semibold text-amber-950">{{ $item->nama_varian }}</td>
                        <td class="p-4 font-medium text-emerald-700">Rp {{ number_format($item->harga, 0, ',', '.') }}</td>
                        <td class="p-4">
                            @if($item->stok <= 5)
                        ...
                        <td class="p-4 text-center pr-6 space-x-2">
