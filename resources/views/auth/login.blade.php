    ...
    <div class="w-full max-w-xl bg-white rounded-[32px] shadow-2xl border border-amber-100/80 overflow-hidden relative z-10 my-4">
        <div class="p-8 md:p-10">
            ...
            <div class="text-center mb-8">
                <h2 class="text-3xl font-bold text-amber-900 tracking-tight">Selamat Datang</h2>
                <p class="text-gray-500 text-sm mt-1">Silakan masuk untuk melanjutkan pesanan kopi favoritmu.</p>
            </div>

            @if (session('status'))
                <div class="mb-4 font-medium text-sm text-green-600 bg-green-50 p-3 rounded-xl border border-green-200 text-center">
                    {{ session('status') }}
                </div>
            @endif
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-5">
                ...
                <div class="flex flex-col gap-3 items-center pt-4 border-t border-gray-100 mt-4">
                    ...
                    <a href="{{ route('password.request') }}" class="text-xs text-amber-700/80 hover:text-amber-900 hover:underline transition flex items-center gap-1">
